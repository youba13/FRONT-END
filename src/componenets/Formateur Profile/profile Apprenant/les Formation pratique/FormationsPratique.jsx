...
import FormationPratiqueTable from './formationcourdesoutien';
...
function FormationPratique() {
  ...
  useEffect(() => {
   const fetchItems = async () => {
       try {
           // An array of IDs
           const idsArray = apprenant.groups;
           // Convert array to a comma-separated string
           const ids = idsArray.join(',');

           const response = await axios.get(`http://localhost:3500/group/${ids}`);
  ...
  return (
    ...
    <div className='h-96 border   overflow-x-auto overflow-y-auto'>
     {(groups.length > 0)?<FormationPratiqueTable></FormationPratiqueTable>:<NoData></NoData>}
  </div>
  </div>
</div>
...
export default FormationPratique