...
function PaiementTable({userid}) {
    ...
    const fetchUser = async () => {
        try {
            const response = await fetch(`http://localhost:3500/formateur/${userid}`); // Change the URL as needed
                ...
                <tbody className='text-black border '>
                    {user.paiements.map(paiement => (
                        <tr key={paiement._id}>
                            ...
                            <td  scope="col" className="px-6 py-3">{paiement.salair}</td>
    ...
    const deletePaiement = async (paiementId) => {
        try {
            const response = await fetch(`http://localhost:3500/formateur/paiementformateur/${paiementId}/${userid}`, {
                method: 'DELETE',
            });
            if (response.ok) {
