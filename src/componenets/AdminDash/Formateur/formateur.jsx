...
import FormateurInfo from './apprenantInfo';
...
function Formateur() {
    ...
    const handleDelete = async (apprenantId) => {
      try {
        const response = await axios.delete(`http://localhost:3500/formateur/${apprenantId}`);
    ...
    const handleShowForm = () => {
      ...
      const handleShowInfo = (formateur) => {
        const form = <FormateurInfo formateur={formateur}></FormateurInfo>
        setModalContent(form);
        setModalOpen(true);
      };
  ...
  const fetchData=  async () => {
    fetch('http://localhost:3500/formateur')
  ...
  <div>
    ...
    <div className="flex items-center justify-between p-5 text-lg w-full font-semibold text-left rtl:text-right text-gray-900 bg-white ">
      <h1> Les Formteurs :</h1>
      <button onClick={()=>handleShowForm()} type="button" className="text-yellow-500 bg-blue-900 hover:bg-blue-950 focus:ring-4 font-bold focus:outline-none focus:ring-primary-300 py-2.5 px-5 me-2 mb-2 text-sm  rounded-lg border border-gray-200  focus:z-10  focus:ring-gray-100 ">Ajouter Formateur</button>
  ...
  <thead class="text-xs text-yeallow-400 uppercase bg-blue-950 text-yellow-500">
                    <tr>
    ...
    <tbody>
      ...
      {apprenants?apprenants.map(apprenant =>(
      <tr key={apprenant._id} className="text-black text-bold  bg-white">
      <td className="px-6 py-4">{apprenant.nom + " "+apprenant.prenom}</td>
        <td className="px-6 py-4">{apprenant.adress}</td>
        <td className="px-6 py-4">{apprenant.email}</td>
        <td className="px-6 py-4">{apprenant.specialite}</td>
        <td className="px-6 py-4">{apprenant.numTelephone}</td>
        <td className="px-6 py-4">{apprenant.groups.length}</td>
