...
function FormationDeLangueAdmin() {
  ...
  useEffect(() => {
      const fetchSalles = async () => {
          try {
              const response = await axios.get('http://localhost:3500/formation/formationDeLangue');
  ...
  const  deleteItem= async (id)=> {
    try {
        const response = await fetch(`http://localhost:3500/formation/formationDeLangue/${id}`, {
            method: 'DELETE'
        });
        if (response.ok) {
...
<div className="flex items-center justify-between p-5 text-lg w-full font-semibold text-left rtl:text-right text-gray-900 bg-white ">
  <h1 className='text-bold'> Les Formations Des Langues:</h1>
                ...
                <tbody>
                    {salles.map(item => (
                        <tr key={item._id}>
                            ...
                            <td className="px-6 py-4">{item.langue}</td>
...
export default FormationDeLangueAdmin