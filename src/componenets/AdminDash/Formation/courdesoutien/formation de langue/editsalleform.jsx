...
function EditSalleForm({onClose ,id}) {
    ...
    const handleInputChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`http://localhost:3500/salle/${id}`, salle);
            console.log('Registration Success:', response.data);
            alert("La Salle a été modifiée");
      ...
      useEffect(() => {
        const fetchSalle = async () => {
            try {
                const response = await axios.get(`http://localhost:3500/salle/${id}`);
                setSalle(response.data); // assuming the server responds with an array of demandes
