...
function CourDeSoutien() {
  ...
  useEffect(() => {
      const fetchSalles = async () => {
          try {
              const response = await axios.get('http://localhost:3500/formation/courDeSoutien');
  ...
  const  deleteItem= async (id)=> {
    try {
        const response = await fetch(`http://localhost:3500/formation/courDeSoutien/${id}`, {
            method: 'DELETE'
        });
        if (response.ok) {
...
<div className="flex items-center justify-between p-5 text-lg w-full font-semibold text-left rtl:text-right text-gray-900 bg-white ">
  <h1 className='text-bold'> Les Formations Cours de Soutien:</h1>
            ...
            <table  className=" w-full text-lg  text-left rtl:text-right text-black-500 ">
                <thead className="text-xs text-yeallow-400 uppercase bg-blue-950 text-yellow-500">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">Niveau Académique</th>
                ...
                <tbody>
                    {salles.map(item => (
                        <tr key={item._id}>
                            ...
                            <td className="px-6 py-4">{item.niveau} {item.filiere} {item.annee}</td>
