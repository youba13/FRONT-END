...
function TimetableDisplay({groupName}) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchEvents, 1000); // Fetch every 5 seconds
                    ...
                    <td key={day} className="p-2 md:border  md:border-gray-500 text-left md:table-cell">
                      {events.filter(event => event.day === day).map(event => (
                        <div key={event._id}  className=" rounded-lg text-white relative text-bold hover:bg-blue-700 bg-blue-600 p-4 my-1">
                          <p>{`${groupName} , La Salle ${event.room}`}</p>
                          <p>Formateur:<span className='text-yellow-500 text-medium'>{event.formateur}</span> </p>
                          <p>{formatTime(event.startTime)} - {formatTime(event.endTime)}</p>
                          <button onClick={()=> deleteEvent(event._id)} className='bg-red-400 rounded-full absolute bottom-1 right-1 px-2 hover:bg-red-600'>x</button>
                        </div>
                      ))}
                    </td>
                  ))}
